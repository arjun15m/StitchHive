...
app.route('/catalog/:email')
  ...
  // Add a new route to handle hiring a tailor
  app.post('/hire-tailor', async (req, res) => {
    try {
      const { b_email, t_email } = req.body;
  
      // Get the IDs for the boutique owner and tailor based on their emails
      const boutiqueResult = await pool.query('SELECT id FROM boutique WHERE b_email = $1', [b_email]);
      const tailorResult = await pool.query('SELECT t_id FROM tailor WHERE t_email = $1', [t_email]);
  
      if (boutiqueResult.rows.length === 1 && tailorResult.rows.length === 1) {
        const b_id = boutiqueResult.rows[0].id;
        const t_id = tailorResult.rows[0].t_id;
  
        // Insert the hiring information into the "hire" table
        const hireQuery = `
          INSERT INTO hire (t_id, b_id, hire_date)
          VALUES ($1, $2, NOW())
          RETURNING h_id;
        `;
  
        const hireResult = await pool.query(hireQuery, [t_id, b_id]);
  
        res.status(201).json({ message: 'Tailor hired successfully', h_id: hireResult.rows[0].h_id });
      } else {
        res.status(404).json({ message: 'Boutique owner or tailor not found' });
      }
    } catch (error) {
      console.error('Error hiring tailor:', error);
      res.status(500).json({ message: 'Error hiring tailor' });
    }
  });
